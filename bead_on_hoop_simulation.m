...
function dydt = bead_on_hoop_dynamics(~, y, params)
    ...
    hoop_angle_rate = bead_momentum / (params.bead_mass * ... 
        params.hoop_radius ^ 2);
    
    bead_momentum_rate = params.bead_mass * ... 
        params.gravitational_acceleration * params.hoop_radius * ... 
        sin(hoop_angle) - params.bead_mass * params.hoop_radius ^ 2 * ... 
        params.hoop_angular_velocity ^ 2 * cos(hoop_angle) * ... 
        sin(hoop_angle);

    dydt = [hoop_angle_rate; bead_momentum_rate];
end
...
function plot_and_save_results(t, y, params)
    ...
    plot(t, y(:, 1));
    xlabel('Time (s)');
    ylabel('Hoop Angle (rad)');
    title('Time History of Hoop Angle');
